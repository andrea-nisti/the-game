    ...
    http::request<Body, http::basic_fields<Allocator>>&& request)
{
    ...
    auto make_server_error_response = [&request](beast::string_view error)
    {
        ...
        response.set(http::field::content_type, "text/html");
        response.keep_alive(request.keep_alive());
        response.body() = "An error occurred: '" + std::string(error) + "'";
        response.prepare_payload();
        return response;
    };
    ...
    if (request.method() != http::verb::get && request.method() != http::verb::head)
        return make_bad_request_response("Unknown HTTP-method");

    // Check if the request path is valid
    if (request.target().empty() || request.target()[0] != '/' ||
        request.target().find("..") != beast::string_view::npos)
        return make_bad_request_response("Illegal request-target");
    ...
    file_body.open(path.c_str(), beast::file_mode::scan, ec);
    ...
    if (ec)
        return make_server_error_response(ec.message());

    // Cache the size since we need it after the move
    auto const file_size = file_body.size();
