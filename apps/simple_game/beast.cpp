...
class TestListener : public TcpListenerBase
{
  ...
  private:
    ...
    std::unique_ptr<RouteManagerBase<http::request<http::string_body>, http::response<http::string_body>>>
        route_manager_ =
            RouteManagerBuilder<ReqT, ResT>{}
                .Add(ConvertVerbBeast(http::verb::get),
                     ...
                     [](const ReqT& req, std::optional<Params> params, ResT& res) -> void {
                         ...
                         res_body += "Params: {";
                         ForEachParam(params, [&](const std::string& key, const std::string& value) {
                             res_body += "\n  \"" + key + "\" : \"" + value + "\",";
                         });
                         res_body += "\n}";
                ...
                .Add(ConvertVerbBeast(http::verb::post),
                     "/test/set_state",
                     [this](const ReqT& req, std::optional<Params> params, ResT& res) -> void {
                         state_ = req.body();
                         ...
                         for (auto& ctx : ctxs_)
                         {
                             if (auto s = ctx.second->ws_session.lock(); s)
                             {
                                 if (params.has_value() and params.value().count("delay"))
                                 {
                                     ...
                                     try
                                     {
                                         delay_secs = std::stoi(params.value().at("delay"));
                                     }
                                     catch (const std::exception& e)
                                     {
                                         std::cerr << "Failed to parse delay: " << e.what() << std::endl;
                                     }

                                     auto timer = std::make_unique<boost::asio::steady_timer>(GetIOContextRef());
                                     timer->expires_after(std::chrono::seconds(delay_secs));

                                     timer->async_wait([s, this, timer = std::move(timer)](
                                                           const boost::system::error_code ec) { s->Send(state_); });
                                 }
                                 else
                                 {
                                     s->Send(state_);
...
auto main(int argc, char* argv[]) -> int
{
    ...
    TestListener listener{io_context, tcp::endpoint{tcp::v4(), static_cast<net::ip::port_type>(port)}};
    listener.Start();
    io_context.run();
